export const updateUIP = async (dataSet,url) => {
    console.log('Update UI with predict weather of the travel date')
    ...
    try{
        ...
        document.getElementById('body').innerHTML =`<div>Temperature(C): </b>${uiData[uiData.length-1].tempHigh} ~ ${uiData[uiData.length-1].tempLow}<br><b> Humidity: </b>${uiData[uiData.length-1].humidity}%<br><b>UV Index:</b> ${uiData[uiData.length-1].uvIndex}.<br><button onClick="window.location.reload();" style="font-size:15px">Remove the Trip or Start a New Trip</button></div>`
